...
\textbf{Optimistic Rollups} work on the principle that all off-chain transactions are valid by default, allowing high throughput and reduced latency compared to immediate verification on Layer 1. Transactions are grouped into batches and processed off-chain by operators, who then submit a proposed new state to Layer 1. After submission, there is a defined \emph{challenge period} during which any participant can contest a transaction in the batch using a \emph{fraud proof}.
